...
test.describe("the lists index", () => {
  ...
  test("filtering by media", async ({ page }) => {
    ...
    await page.getByLabel("Media minimum").fill("10")
    await page.getByRole("button", { name: "Submit"}).click()
    await expect(page.getByRole("heading", {name: "Public list"})).toBeVisible()
    await expect(page.getByRole("heading", { name: "test list"})).not.toBeVisible()
  })
})
...
test.describe("as a logged in user", () => {
  ...
  test.describe("creating a copy of a list", () => {
    ...
    test("Creating a copy of a list", async ({ page }) => {
      ...
      await expect(page.getByAltText("The Fairly OddParents", {exact: true})).toBeVisible()
      await expect(page.getByAltText("The Fairly OddParents: Fairy Idol", {exact: true})).toBeVisible()
    })
