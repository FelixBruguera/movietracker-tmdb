...
describe("The filters validation", () => {
  it("doesn't allow the maximum year to be before the minimum year", () => {
    const result = moviesSchema.safeParse({...defaultParams, "primary_release_date.gte": 1999, "primary_release_date.lte": 1995})
  ...
  it("doesn't allow the maximum average rating to be less than the minimum", () => {
    const result = moviesSchema.safeParse({...defaultParams, "vote_average.gte": 8, "vote_average.lte": 5})
    expect(result.success).toBe(false)
  ...
  it("only accepts strings that are json parseable in the with_keywords param", () => {
    let result
    try {
      result = moviesSchema.safeParse({...defaultParams, "with_keywords": "<script>console.log('Hello')</script>"})
    }
    catch (e) {
      result = e
    }
...
describe("The GET /movies/:id endpoint", () => {
  ...
  it("doesn't fetch the external api when there's a cache hit", async () => {
    const scopedEnv = createTestEnv()
    scopedEnv.KV.get = () => Promise.resolve(JSON.stringify({ data: "mocked data" }))
    const request = new Request(baseUrl, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    })
