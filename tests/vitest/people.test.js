 ...
 const baseUrl = "http://localhost/api/people/25"
    ...
    beforeEach(() => {
     ...
     it("writes to the cache after fetching the external api", async () => {
        ...
        expect(env.KV.put).toHaveBeenCalledWith(expect.any(String), JSON.stringify(responseBody), expect.objectContaining({ expirationTtl: 604800}))
    })
...
describe("The GET /people/:person/credits endpoint", () => {
 const baseUrl = "http://localhost/api/people/25/credits"
 const mockData = {
        cast: [{id: 1, character: "Lorem", vote_average: 5, vote_count: 50, release_date: "2025-03-03"}, {id: 2, character: "Ipsum", vote_average: 8, vote_count: 10, release_date: "2025-01-03"}],
        crew: [{id: 3, department: "Camera", vote_average: 9, vote_count: 9, release_date: "2022-03-03"}, {id: 4, department: "Production", vote_average: 3, vote_count: 5, release_date: "2005-01-31"}, {id: 5, department: "Directing", vote_average: 5, vote_count: 100, release_date: "2015-11-18"}]    
    }
    const env = createTestEnv()
    beforeEach(() => {
        vi.clearAllMocks()
    ...
    it("filters by all departments", async () => {
        ...
        const response = await app.fetch(request, env)
        const responseBody = await response.json()
        expect(responseBody.results).toStrictEqual([...mockData.cast, ...mockData.crew].sort((a, b) => new Date(b.release_date) - new Date(a.release_date)))
    })
    ...
    it("writes to the cache after fetching the external api", async () => {
        ...
        expect(axios.get).toHaveBeenCalledTimes(1)
        expect(env.KV.put).toHaveBeenCalledTimes(1)
        expect(env.KV.put).toHaveBeenCalledWith(expect.any(String), expect.stringContaining("cast"), expect.objectContaining({ expirationTtl: 86400}))
    })
    it("doesnt fetch the external api when there's a cache hit", async () => {
