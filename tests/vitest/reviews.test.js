...
import { deleteReview, getUserReview, insertReview, likeReview, updateReview } from "../../src/api/reviews/queries"
import * as schema from "../../src/db/schema"

const sqlite = new Database(".wrangler/state/v3/d1/miniflare-D1DatabaseObject/1298067f57f93ced47ab7edeef6597f06c319d51a9c7266417d3fa0df0acf884.sqlite")
const db = drizzle(sqlite, { schema })

describe("The media reviews validation", () => {
  it("fails with unallowed sort_by values", () => {
    expect(reviewsSchema.safeParse({ sort_by: "id", sort_order: "1"})).toEqual(expect.objectContaining({success: false}))
  })
...
describe("The new review validation", () => {
  ...
  it("doesnt allow negative ratings", () => {
    const result = newReviewSchema.safeParse({rating: -1, text: "new review", movie: mockMovie})
  ...
  it("doesnt allow ratings bigger than 10", () => {
    const result = newReviewSchema.safeParse({rating: 11, text: "new review", movie: mockMovie})
    expect(result).toEqual(expect.objectContaining({success: false}))
    expect(result.error.issues[0].path).toEqual(["rating"])
    expect(result.error.issues[0].code).toEqual("too_big")
  })
  it("allows empty text", () => {
    const result = newReviewSchema.safeParse({rating: 5, text: "new review", movie: mockMovie})
    expect(result).toEqual(expect.objectContaining({success: true}))
  })
  it("doesnt allow more than 400 characters of text", () => {
    ...
    const result = newReviewSchema.safeParse({rating: 6, text: longText, movie: mockMovie})
    expect(result).toEqual(expect.objectContaining({success: false}))
    expect(result.error.issues[0].path).toEqual(["text"])
    expect(result.error.issues[0].code).toEqual("too_big")
  })
  it("defaults addToDiary to false", () => {
    const result = newReviewSchema.safeParse({rating: 3, text: "test", movie: mockMovie})
...
describe("The insertReview query", () => {
  it("doesn't allow duplicated reviews", async () => {
    let result
    try {
      result = await insertReview(db, "test", 1, "8o0E4dFL8bigp2A35UjKEjFVfr03KOuS", 755898)
    }
    catch (e) {
      result = e
    }
...
describe("The updateReview query", () => {
  it("doesn't allow to update another user's review", async () => {
    const result = await updateReview(db, "test", 1, 21, "BlIFKjnfQ0HhK0HCJ2B7YXM9YapbEDpU")
    expect(result).toEqual([])
  })
})
...
describe("The deleteReview query", () => {
  it("doesn't allow to delete another user's review", async () => {
    const result = await deleteReview(db, 21, "BlIFKjnfQ0HhK0HCJ2B7YXM9YapbEDpU")
