...
import { deleteList, deleteListMedia, followList, getList, getListMedia, getListMediaIds, getListMediaTotal, getListOwner, getLists, getListsTotal, getUserLists, insertList, insertListMedia, unfollowList, updateList } from "./queries"
...
app.get("/", async (c) => {
    ...
    const [listsIndex, aggregates] = await Promise.all([
        getLists(db, minMedia, minFollowers, userId, filterCondition, sort, offset, itemsPerPage),
        getListsTotal(db, userId)
    ])
...
app.get("/:listId/media", async (c) => {
  ...
  const sort = getSort({ sort_order: sort_order, sort_by: "date"}, { date: mediaToLists.createdAt})
  const offset = page * itemsPerPage - itemsPerPage
  const [listMedia, aggregations] = await Promise.all([
    getListMedia(db, listId, sort, offset, itemsPerPage),
    getListMediaTotal(db, listId)
  ])
...
app.post("/", auth, async (c) => {
  ...
  const db = drizzle(c.env.DB, { schema: schema })
  try {
    const result = await insertList(db, crypto.randomUUID(), validation.data, session.user.id)
...
app.post("/:listId/media", auth, async (c) => {
  ...
  try {
    ...
    else if(e.cause.message.includes("Request failed with status code 404")) {
