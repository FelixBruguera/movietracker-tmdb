import { and, countDistinct, eq, gte, inArray, or, count, sql } from "drizzle-orm"
import { listFollowers, lists, media, mediaToLists, user } from "../../db/schema"
...
export function getLists(db, minMedia, minFollowers, userId, filterCondition, sort, offset, itemsPerPage) {
  return db.select({
      id: lists.id,
      name: lists.name,
...
export function getListsTotal(db, userId) {
  return db.select({
...
export function getListOwner(db, listId, userId) {
  return db.select({ userId: lists.userId })
  .from(lists)
...
export function getListMediaTotal(db, listId) {
  return db.select({
...
export function followList(db, listId, userId) {
  return db.insert(listFollowers)
  .values({
    listId: listId,
    userId: userId,
  })
...
export function unfollowList(db, listId, userId) {
  return db.delete(listFollowers)
  .where(
    and(
      eq(listFollowers.listId, listId),
      eq(listFollowers.userId, userId),
    ),
  )
}