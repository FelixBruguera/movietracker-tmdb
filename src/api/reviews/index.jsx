...
import { deleteReview, dislikeReview, getMediaReviews, getUserReview, insertGenreMedia, insertMedia, insertNetwork, insertNetworkMedia, insertPerson, insertPersonMedia, insertReview, likeReview, updateReview } from "./queries"
...
app.get("/:mediaId", async (c) => {
  ...
  const result = await getMediaReviews(db, userId, mediaId, sort, offset, itemsPerPage)
...
app.post("/", auth, async (c) => {
  ...
  const queries = [
    ...
    ...moviePeople.map(person => insertPersonMedia({db: db, personId: person.id, mediaId: movie.id, isDirector: person.job === "Director" })),
...
app.post("/tv", auth, async (c) => {
  ...
  const queries = [
    ...
    ...cast.map(person => insertPersonMedia({ db: db, personId: person.id, mediaId: show.id })),
    ...networkData.map(network => insertNetwork(db, network)),
    ...networkData.map(network => insertNetworkMedia(db, network.id, show.id)),
    ...creators.map(person => insertPersonMedia({db: db, personId: person.id, mediaId: show.id, isCreator: true })),
...
app.patch("/:id", auth, async (c) => {
  const validation = newReviewSchema.extend({
    movie: z.object({ 
