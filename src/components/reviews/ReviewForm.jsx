...
const ReviewForm = ({ previousReview, movie, mutation }) => {
  ...
  return (
    ...
    >
      <Textarea
        className="border-2 bg-input dark:bg-transparent border-ring w-full lg:w-full h-fit max-h-45 field-sizing-content transition-colors"
        name="text"
        value={text}
      ...
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center w-8/10 gap-10 justify-start">
          ...
          <div className="flex items-center justify-center gap-2 w-1/3">
            ...
            >
              <SelectTrigger
                className="w-2/3 lg:w-3/5 text-xs lg:text-sm border-1 bg-card dark:bg-card border-border"
                title="Your Rating"
              >
                {rating}
              </SelectTrigger>
