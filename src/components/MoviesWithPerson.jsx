...
const MoviesWithPerson = () => {
    ...
    const { data: paramData, isLoading: paramDataLoading, isError: paramDataError } = useQuery({
        ...
        queryFn: () => axios.get(`/api/people/${person}`).then(response => response.data)
    })
    const { data, isLoading, isError } = useQuery({
        queryKey: ["movies", searchParams.toString()],
        queryFn: () => axios.get(`/api/movies/people/${person}`, { params: searchParams}).then(response => response.data)
    })
    console.log(data)
