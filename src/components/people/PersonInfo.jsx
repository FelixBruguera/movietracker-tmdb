import { BriefcaseBusiness, Cake, MapPin, SeparatorHorizontal } from "lucide-react"
...
const PersonDetail = ({ name, detail}) => {
  ...
  return (
    <li className="flex flex-col w-full items-start hover:bg-card-bg px-2 py-1 rounded-md group transition-colors">
      <h3 className="text-base font-semibold border-b-2 border-b-stone-300 dark:border-b-stone-700 group-hover:border-b-accent dark:group-hover:border-b-accent transition-colors">{name}</h3>
  ...
  return (
    <div className="mb-5 lg:mb-0">
      <div className="flex flex-col lg:flex-row items-center lg:items-start gap-3 w-full px-5">
        <img
          src={data.profile_path ? `${imageBaseUrl}${data.profile_path}` : "/person-fallback.webp"}
        ...
        <ul className="flex flex-row lg:flex-col items-start lg:items-end justify-evenly lg:justify-end gap-2 w-full lg:w-1/4">
