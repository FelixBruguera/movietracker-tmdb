...
const Collection = ({}) => {
    ...
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: () => axios.get(`/api/movies/collection/${id}`).then(response => response.data)
    })
    ...
    return (
        <div className="min-h-dvh py-5">
            ...
            <div className="relative w-full bg-no-repeat bg-center aspect-16/9 lg:aspect-16/4 flex items-center justify-start" style={{ backgroundImage: `url(${backdropUrl})`}}>
                ...
                <div className="flex items-center justify-center gap-3 mt-3 mx-auto w-full">
                    ...
                    <div className="flex flex-col items-start gap-2 w-lg">
                        ...
                        {data.overview && <p className="text-muted-foreground text-base text-justify max-w-9/10 lg:w-full">{data.overview}</p>}
                    </div>
                </div>
            <ListHeading>
            ...
            {/* <PosterList movies={data.parts} size="base" /> */}
            {data.parts?.map((movie) => {
              return (
                <Link to={`/movies/${movie.id}`}>
                    <MediaCard id={movie.id} title={movie.title} src={movie.poster_path} overview={movie.overview}>
                      {/* {airYear && (
